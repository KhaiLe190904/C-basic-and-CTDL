...
void phongThoaMan(node* hashTable[], int tableSize) {
    ...
    for (int i = 0; i < tableSize; i++) {
        ...
        node* prev = NULL;
        while (current != NULL) {
            if (!((current->ht.people > 0 && current->ht.kids == 0) || (current->ht.people - current->ht.kids > 0 && current->ht.kids > 0))) {
                if (prev == NULL) {
...
void xoaPhong(node* hashTable[], int maPhong, int tableSize) {
    int index = hash(maPhong, tableSize);
    node* current = hashTable[index];
    node* prev = NULL;
    while (current != NULL) {
...
int main() {
    ...
    while (1) {
        ...
        else if (strcmp(lc, "deleteRoom") == 0) {
            int maP;
            scanf("%d", &maP);
            xoaPhong(hashTable, maP, tableSize);
        }
    }
    return 0;
}